...
categoryRouter.put('/api/categories:id',jsonParser,(request,response,next)=>{
  ...
  return Category.findByIdAndUpdate(requestparams.id,request.body,options)
  .then((updatedCategory)=>{
      if(!updatedCategory){
          logger.log(logger.ERROR,'CATEGORY ROUTER: responding with 404 status code-no updatedCategory');
          return next (new HttpErrors(404,'category not found'));
      }
      logger.log(logger.INFO,`GET-responding with 200 code`);
...
categoryRouter.get('api/categories/:id',(request,response,next)=>{
    ...
    .then((category)=>{
        if(!category){
            logger.log(logger.ERROR,'CATEGORY ROUTER: responding with 404 status code-no category');
            return next(new HttpErrors(404,'category not found'));
        }

    logger.log(logger.INFO,'CATEGORY ROUTER: responding with 402');
